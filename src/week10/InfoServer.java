    ...
    public InfoServer() {
        ...
        try {
            ...
            while (true) {
                ...
                outToClient.writeBytes("InfoServer versi 0.1\n" + "hanya untuk testing..\n" + "Silahkan berikan perintah TIME | NET | QUIT | IPF | SCT\n");
                ...
                while (!isQUIT) {
                    ...
                    if (datafromClient.startsWith("TIME")) {
                        ...
                        outToClient.writeBytes(InetAddress.getByName("www.google.com").toString()+ "\n");
                    } else if (datafromClient.startsWith("QUIT")) {
                        isQUIT = true;
                    } else if (datafromClient.startsWith("IPF")) {
                        String host = datafromClient.split(" ")[1];
                        outToClient.writeBytes(InetAddress.getByName(host).toString()+ "\n");
                    } else if (datafromClient.startsWith("SCT")) {
                        outToClient.writeBytes(serverSocket.getLocalPort() + "\n");
                    }
                }
                outToClient.close();
                inFromClient.close();
